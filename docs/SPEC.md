...
| `capacity_from_pool` | boolean | default false; when true the slots this availability materialises are written with `capacity` null — measured by the host pool (D18) |
...
| `capacity` | unsigned smallint **nullable** | default 1. A number is the cap, whatever the pool says; **null** = derived from the availability's host pool (D18) |
...
- **PublishAvailability**: `draft → published`; materializes slots. Generation: from `starts_at`, place
  ...
  trailing time is unused. Idempotent (re-publish regenerates nothing if slots exist). Each slot is
  written with `capacity` 1, or null when the availability's `capacity_from_pool` is set — the one
  place that decides which kind of time this day opens (D18), read again by every later regeneration
  of the grid.
...
- **BookSlot(slot, bookedFor, bookedBy, options)** — the concurrency-critical path:
  ...
  2. Validate: slot `open` (or `held` when reached via AcceptOffer), availability `published`, slot
     ...
     remaining capacity > 0 — where a numbered `capacity` column is the cap and a null one means
     the number of people the availability's pool resolves to who are free across the slot (D18).
...
check as a values list, so it is no longer a single statement — but the number of statements **the
package itself** issues is a fixed handful and does not grow with the number of slots. The bound
`HostResolver` is a consumer's code and usually queries: it is asked once per distinct
`(pool entry, context, availability date)` — **not** once per pool row per slot, and not once per
pool row per availability — memoised for the length of the one read in `Support\HostResolution`.
Two roles naming one position, and two blocks of the same day, are one question; a second date is a
second question, because who holds a position is a fact about a day. Nothing is remembered across
reads. It is off by default, so `bookable()` on its own means
exactly what it meant before. `Slot::capacityFor($now)` is the same question asked of one slot as a
number: a numbered `capacity` column verbatim, or — when the column is null — the people its pool
resolves to with nothing else booked across it, falling back to one appointment when there is no
pool to derive from (D18). The rule lives in
`Support\SlotCapacity` and is the **one** definition of a slot's capacity: `BookSlot` gates and
settles on it (D18), and `Support\ReleaseSlot` settles a slot back against it when a booking is
cancelled or an offer lets go, so a cancellation on a two-of-three pooled slot cannot leave it
looking full. The booking and release paths ask with `exclusive_hosts` forced off, because they
...
- **MaterialiseSeries(Series, `$through`)** — for each local date the cadence admits
  ...
  horizon and meta, and `capacity_from_pool` set — a time a rule lays down is measured by how many
  of its pool are free (D18), through every later regeneration of the day. Idempotent: a `(series_id, occurs_on, window_index)` key that already
...
| R25 | CreateOffer refuses capacity>1 slots (D12); a pool-derived slot (null column) is held whole, as it was before the column could be null | `CreateOffer` → `SlotNotOfferable` for a numbered capacity above 1 | `CreateOfferTest`, `SeriesLifecycleTest` "does not put a time back on sale when its offer lapses while the series is paused" (a series-made, pool-derived slot held by an offer) | Done |
...
| R57 | `RegenerateSeries` remakes stale future following occurrences, never deletes one carrying a live claim (reshaped in place where the new rule covers it — R82 — and left standing where it does not), and releases one whose bookings are all spent | `Actions\RegenerateSeries` | `UpdateSeriesTest` live-booking, reshaping and released-day cases | Done |
| R58 | `FindSeriesConflicts` names the live future bookings a proposed rule would strand and nothing else, judged by block index (R77); a shorter horizon is not a conflict; past and detached days are ignored | `Actions\FindSeriesConflicts` | `UpdateSeriesTest` conflict cases | Done |
...
| R60 | Pause retires unclaimed future slots (detached days included) and keeps booked ones; resume reopens the same rows on days that are still published, without duplicating, then regenerates out to the horizon | `Actions\PauseSeries`, `Actions\ResumeSeries` | `tests/Feature/Series/SeriesLifecycleTest.php` pause / resume cases | Done |
| R61 | `DetachOccurrence` takes a day out of the rule's hands and refuses a non-occurrence; `FollowSeries` puts it back through `RegenerateSeries` (R80 for a paused or ended rule); `RemoveOccurrenceWindow` empties a date's block for good (R83) | `Actions\DetachOccurrence`, `Actions\FollowSeries`, `Actions\RemoveOccurrenceWindow` | `SeriesLifecycleTest` detach / follow / removed-window cases | Done |
| R62 | `DeleteSeries` is refused if any occurrence ever carried a booking, released ones included (R71); otherwise rule, windows, pool and occurrences all go | `Actions\DeleteSeries` | `SeriesLifecycleTest` delete cases | Done |
| R63 | `SweepSeries` rolls every active series to its horizon **through `RegenerateSeries`** (R72), retires past unclaimed slots, and ends a series whose `ends_on` has passed on the series' own calendar | `Actions\SweepSeries` | `SeriesLifecycleTest` sweep cases (incl. the ward-clock end date and the paused series) | Done |
...
| R67 | `BookSlot` gates a **null-capacity** slot on who is free, not on the `capacity` column: one whose pool resolves to three free people takes three claims and refuses the fourth, flipping to `booked` only on the third; a pool that resolves to nobody, or whose only holder is booked across the slot, refuses the first; a numbered capacity, and a null one with no pool behind it, gate on the column. Two connections contending for a null-capacity slot with two free people both win; with one free person exactly one does | `Support\SlotCapacity::forClaim()` (used by `BookSlot`'s capacity check and settle step, and by `Support\ReleaseSlot`), `Slot::capacityFor()` | `tests/Feature/Booking/PooledCapacityTest.php`, `tests/Concurrency/BookSlotConcurrencyTest.php` pooled cases | Done |
| R68 | `config('dibs.exclusive_hosts')` (default `false`) makes a booking on the **pool-derived** slot being asked about count against its own host, in one definition (`Support\OverlapCheck::hostsAreExclusive()`) that `freeHolders`, `capacityFor()`, `bookable(requireFreeHost:)` and `AssignBookingHost(guardHostOverlap: true)` all read. Default off leaves the R19 behaviour exactly as it was | `Support\OverlapCheck`, `Support\HostAvailability::busyAssignments()`, `Slot::scopeBookable()`, `config/dibs.php` | `PooledCapacityTest.php` exclusive-host cases | Done |
...
| R84 | A numbered `capacity` is the cap wherever capacity is read, whatever the availability's pool says: a capacity-1 slot pooled on three free people takes exactly one claim and then reads `booked`; `capacityFor()` on a capacity-2 pooled slot is 2 however many holders are free; `bookable(requireFreeHost: true)` still drops it only when nobody in the pool is free; and with `exclusive_hosts` on, its own claim does not take its host out of that filter | `Support\SlotCapacity::of()` (the column first), `Slot::scopeBookable()` (own-slot claims count only for a null capacity) | `PooledCapacityTest` "takes exactly one appointment on a candidate-list time…", "reports a numbered capacity from the column and a null one from the pool", "offers a numbered-capacity time only while somebody in its pool is free", "leaves a numbered-capacity time on offer when its own claim is all that is against its host" | Done |
| R85 | `dibs_availabilities.capacity_from_pool` decides which kind of time a grid lays down, and every path that lays one down reads it: `PublishAvailability` and `UpdateAvailabilityGeometry` write `capacity` null when it is set, `MaterialiseSeries` sets it on every occurrence it creates, `DuplicateAvailability` carries it, so a series-made time is pool-derived and comes back pool-derived from every regeneration | `Availability::slotCapacity()`, `Actions\PublishAvailability`, `Actions\UpdateAvailabilityGeometry`, `Actions\MaterialiseSeries`, `Actions\DuplicateAvailability` | `PublishAvailabilityTest` "leaves the capacity of its times to the host pool when the availability asks it to", `UpdateAvailabilityGeometryTest` "keeps the times pool-derived when the grid is remade", `PooledCapacityTest` "takes one appointment per free holder on a series-made time, and refuses the fourth" | Done |
